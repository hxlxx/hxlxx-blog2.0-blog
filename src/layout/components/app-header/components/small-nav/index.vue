...
import { useSiteProfile, useSmallNav } from '@/stores'
...
const handleClickNav = (path: string) => {
  router.push(path)
  smallNavStore.showSmallNav = false
}
</script>

<template>
  <div
    v-if="smallNavStore.showSmallNav"
    class="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-[rgba(0,0,0,0.2)] z-50"
  >
    <div class="card w-[80%] p-5">
      ...
      <ul class="mt-2 flex flex-col items-center gap-1 family-shuhei">
        <li v-for="nav in navList" :key="nav.label">
          <span @click="handleClickNav(nav.path)">{{ nav.label }}</span>
        </li>
      </ul>
    </div>
  </div>
